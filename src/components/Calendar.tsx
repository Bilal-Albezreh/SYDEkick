...
import { ChevronLeft, ChevronRight, Calendar as CalendarIcon, List, Check, Clock, ExternalLink, Filter, X } from "lucide-react";
...
const COURSE_URLS: Record<string, string> = {
  "SYDE 285": "https://learn.uwaterloo.ca/d2l/home/1229823", // Example ID
...
const getAssessmentType = (name: string) => {
...
export default function Calendar({ initialData }: { initialData: any[] }) {
  // 1. STATE
  ...
  // 3. ACTIONS
  const handleToggleComplete = async (assessmentId: string, currentStatus: boolean) => {
      ...
      <div className="flex flex-col animate-in fade-in duration-500">
        <div className="grid grid-cols-7 flex-1 auto-rows-fr bg-gray-900/20">
          ...
          {daysArray.map(day => {
            ...
            let dayItems = allAssessments.filter(a => a.due_date === dateStr);
            ...
            return (
              <div 
                key={day} 
                onClick={() => {
                    if (visibleItems.length > 0) setSelectedDay(dateStr);
                }}
                className={cn(
              ...
              >
                ...
                {displayItems.map((item: any) => {
                    const typeLabel = getAssessmentType(item.name);
                    return (
                        <div 
                            key={item.uniqueId}
                            className={cn(
                        ...
                        >
                            <div className="flex items-center justify-between mb-1">
                                <span className="text-xs font-black tracking-tight uppercase opacity-95">{item.courseCode}</span>
                            ...
                            <button
                                ...
                                className={cn(
                                    ...
                                    item.is_completed ? "border-current text-current" : "border-white/20 text-transparent hover:border-white/50"
                                )}
                            >
                ...
                {hiddenCount > 0 && (
                   <div className="mt-auto text-[10px] font-bold text-gray-500 text-center py-0.5 bg-gray-800/30 rounded">
                      +{hiddenCount} More
                   </div>
                )}
              </div>
  ...
  const renderListView = () => {
    const itemsByWeek = allAssessments.reduce((acc: any, item: any) => {
    ...
    const uniqueCourses = Array.from(new Set(allAssessments.map(a => a.courseCode)));
            ...
            {sortedWeeks.map(weekNum => {
                ...
                return (
                    <div key={weekNum} className="mb-10">
                        {/* CHANGED: Removed 'sticky top-[85px]' -> 'relative' */}
                        ...
                        <div className="space-y-3">
                            {items.map((item: any) => {
                                ...
                                const isUrgent = daysMsg === "Tomorrow" || daysMsg === "Due Today" || daysMsg === "Overdue";
                                return (
                                    <div key={item.uniqueId} className={cn("group flex items-center gap-6 p-5 rounded-2xl border transition-all duration-200 relative overflow-hidden", item.is_completed ? "bg-transparent border-transparent opacity-40 hover:opacity-100" : "bg-[#141414] border-gray-800 hover:border-gray-700 hover:bg-[#1a1a1a]")}>
                                        <div className="absolute left-0 top-0 bottom-0 w-1 opacity-0 group-hover:opacity-100 transition-opacity" style={{ backgroundColor: getCourseColor(item.courseCode) }} />
                                        <button onClick={() => handleToggleComplete(item.uniqueId, item.is_completed)} className={cn("w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all shrink-0 ml-1", item.is_completed ? "bg-emerald-500 border-emerald-500 text-white scale-110" : "border-gray-600 hover:border-white text-transparent")}><Check className="w-3.5 h-3.5 stroke-[3]" /></button>
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-3 mb-1.5">
                                                ...
                                                <div className="flex items-center gap-1.5 text-sm text-gray-500 font-medium"><Clock className="w-3.5 h-3.5" /><span>{formatDate(item.due_date)}</span></div>
                                            </div>
                                            <div className={cn("text-lg font-medium text-gray-200", item.is_completed && "line-through text-gray-500")}>{item.name}</div>
                                        </div>
                                        <div className={cn("text-sm font-bold px-4 py-2 rounded-lg border ml-auto shrink-0 transition-opacity", item.is_completed ? "opacity-0" : "opacity-100", isUrgent ? "bg-red-950/20 border-red-500/20 text-red-400" : "bg-gray-800 border-gray-700 text-gray-400")}>{daysMsg}</div>
                                    </div>
                                )
                            })}
                        </div>
  ...
  return (
    <div className="flex flex-col h-full bg-[#0a0a0a] rounded-xl overflow-y-auto custom-scrollbar relative border border-gray-900">
      ...
      <div className="sticky top-0 z-50 backdrop-blur-xl bg-black/80 border-b border-white/10 shadow-2xl">
        ...
        {/* INTEGRATED DAYS HEADER (Only for Month View) */}
        {viewMode === "month" && (
            <div className="grid grid-cols-7 text-center pb-2">
                {weekDays.map((d, i) => (
                    <div 
                        key={d} 
                        className={cn(
                            "text-[10px] font-bold text-gray-500 uppercase tracking-widest py-1",
      ...
      {/* 2. MAIN CONTENT (Scrollable behind header) */}
      <div className="flex-1 min-h-0 bg-[#0a0a0a] pb-20"> {/* pb-20 prevents cutoff */}
                             ...
                             >
                                ...
                                <div className="flex-1">
                                    <div className="flex items-center gap-2 mb-1">
                                        <span className="text-[10px] font-bold px-1.5 rounded bg-white/5 text-gray-400" style={{ color: getCourseColor(item.courseCode) }}>{item.courseCode}</span>
