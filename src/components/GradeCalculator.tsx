    ...
    <div className="flex flex-col gap-6">
      ...
      <div 
        ...
        className={cn(
          ...
          viewMode === "overview" ? "bg-blue-900/10 border-blue-500/50 ring-1 ring-blue-500/50" : "bg-[#191919] border-gray-800 hover:border-gray-700"
        )}
      >
        ...
        <div className="flex items-center gap-8 mt-4 md:mt-0">
           ...
           <div className="text-right">
              <div className={cn("text-4xl font-black tabular-nums", termStats.average >= 80 ? "text-green-500" : "text-white")}>{termStats.average.toFixed(1)}%</div>
           </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 h-[650px]">
        ...
        <div className="lg:col-span-1 bg-[#191919] border border-gray-800 rounded-xl overflow-hidden flex flex-col">
           ...
           <div className="flex-1 overflow-y-auto p-2 space-y-1">
              <button onClick={() => setViewMode("overview")} className={cn("w-full flex items-center gap-3 p-3 rounded-lg text-left transition-all", viewMode === "overview" ? "bg-blue-600/10 text-blue-400 font-bold" : "text-gray-400 hover:bg-white/5")}>
              ...
              {courses.map((course) => {
                 ...
                 return (
                    <button key={course.id} onClick={() => { setSelectedCourseId(course.id); setViewMode("detail"); }} className={cn("w-full flex items-center justify-between p-3 rounded-lg text-left transition-all border", isSelected ? "bg-blue-600/10 border-blue-500/50" : "bg-transparent border-transparent hover:bg-white/5 text-gray-400")}>
                        <div className="flex items-center gap-3">
                           <div className="w-1.5 h-8 rounded-full" style={{ backgroundColor: course.color }} />
                           <div className="min-w-0">
                             <div className={cn("font-bold text-sm truncate", isSelected ? "text-white" : "text-gray-300")}>{course.course_code}</div>
                           </div>
                        </div>
                   ...
                   <div className="space-y-3">
                      {sortedAssessments.map((assess: any) => (
                         <div key={assess.id} className={cn(
                             "flex items-center gap-4 p-4 rounded-xl border transition-colors group relative",
                             assess.isDropped 
                               ? "bg-red-900/10 border-red-900/20 opacity-60" 
                               : "bg-black/20 border-white/5 hover:border-gray-700"
                           )}>
