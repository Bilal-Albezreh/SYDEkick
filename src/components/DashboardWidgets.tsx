...
export default function DashboardGrid({ 
  ...
  const upcomingAssessments = courses.flatMap(c => c.assessments)
    .filter((a: any) => !a.is_completed && new Date(a.due_date) > new Date());

  const courseProgress = courses.map(c => {
    // 1. Calculate Total Weight of all assessments
    const totalWeight = c.assessments.reduce((acc: number, curr: any) => acc + curr.weight, 0);
    ...
    const completedWeight = c.assessments
      ...
      .reduce((acc: number, curr: any) => acc + curr.weight, 0);
